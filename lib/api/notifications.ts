...
export const notificationApi = {
  getUserNotifications: async (userId: string): Promise<Notification[]> => {
    const response = await client.get<Notification[]>(`/api/notifications/user/${userId}`);
    return response.data;
  },

  getUnreadNotifications: async (userId: string): Promise<Notification[]> => {
    const response = await client.get<Notification[]>(`/api/notifications/user/${userId}/unread`);
    return response.data;
  },

  markAsRead: async (notificationId: string): Promise<Notification> => {
    const response = await client.patch<Notification>(`/api/notifications/${notificationId}/read`);
    return response.data;
  },
